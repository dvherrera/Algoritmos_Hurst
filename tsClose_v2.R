...
hurst_rs <- function(v) {
  ...
  for(k in 1:(nk)){
    ...
    repeat {
      vector_acumulado <- c(vector_acumulado,subset(v2,vector_indices==i))
      i=i+1
      if(i>d) break
    }
    ...
    #vector_rango <- NULL
    repeat {
      vector_rango_RS[i] <- (max(subset(vector_acumulado,vector_indices==i)) - 
                               min(subset(vector_acumulado,vector_indices==i)))/
                               sd(subset(vector_acumulado,vector_indices==i))
      #print(i)
      i=i+1
      if(i>=d) break
    }
